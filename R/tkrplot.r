...
{ # convert the x,y clicked co-ordinates into values from the plot itself
  ...
  xMin <- parplt[1] * width; xMax <- parplt[2] * width; yMin <- parplt[3] * height; yMax <- parplt[4] * height
  rangeX <- parusr[2] - parusr[1]; rangeY <- parusr[4] - parusr[3]
  xClick <- as.numeric(xClick)+0.5; yClick <- as.numeric(yClick)+0.5; yClick <- height - yClick
  xPlotCoord <- parusr[1]+(xClick-xMin)*rangeX/(xMax-xMin); yPlotCoord <- parusr[3]+(yClick-yMin)*rangeY/(yMax-yMin)
...
rp.tkrreplot <- function(panel, name) {
  ...
  if (!exists(panel$panelname, .rpenv, inherits = FALSE)) # if the panelname is not set then
     ...
     panelname <- panel$panelname
  name <- deparse(substitute(name))
  w.tkrreplot(rp.widget.get(panelname, name))
  invisible(panelname)
}
